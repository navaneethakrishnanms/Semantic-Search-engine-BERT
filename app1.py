...
def search_ui(query, top_k):
    ...
    if isinstance(results, str):
        return results
    return "\n\n".join([f"{i+1}. {sent}\n(Score: {score:.4f})" for i, (sent, score) in enumerate(results)])

demo = gr.Interface(
    fn=lambda paragraph, query, top_k: (add_text_ui(paragraph), search_ui(query, int(top_k))),
    inputs=[
    ...
    description="Add text to corpus and search for similar sentences using Sentence Transformers."
)
